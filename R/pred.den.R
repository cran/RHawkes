...
function(x, tms, cens, par,
  ...
  Snp1 <- exp(-Mu(cens-tms) + Mu(tms[n] - tms) - Phi(cens) + Phi(tms[n]) )
  
  tmp <- mllRH1(tms, cens, par)
  pnp1 <- exp(tmp$log.p[n*(n-1)/2 + 1:n])
  den <- sum(Snp1 * pnp1)
  
  dnp1 <- exp(-Mu(outer(cens + x, tms, "-")) +
              Mu(outer(rep(tms[n], length(x)), tms, "-"))
              - (sapply(cens + x,Phi) - Phi(tms[n]))
              )*(mu(outer(cens + x, tms, "-")) + sapply(cens + x, phi))
  rowSums(dnp1 * outer(rep(1, length(x)), pnp1)) / den
}